...
async function tryMcpQuery(pattern, cwd) {
  ...
  return new Promise((resolve) => {
    let out = '';
    let done = false;
    const child = spawn('npx', ['codebase-vis', 'serve', '--mcp'], { stdio: ['pipe', 'pipe', 'pipe'], cwd });
...
async function main() {
  ...
  const mcpMatches = await tryMcpQuery(pattern, cwd);
  if (mcpMatches && mcpMatches.length > 0) {
    console.log(JSON.stringify({
      permission: 'deny',
      userMessage: `[codebase-vis] Graph already has this answer (no grep needed):\n\n${mcpMatches.join('\n')}\n\nOverride: append "# --graph-tried" to command.`,
    }));
    process.exit(0);
  }

  const graphPath = path.join(cwd, 'codebase-out', 'graph.json');
